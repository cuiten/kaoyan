...
@Service
@Transactional
public class PdtServiceImp implements PdtService {
	...
	public void delete(Integer uuid) {
		sessionFactory.getCurrentSession().delete(
				sessionFactory.getCurrentSession().load(Pdt.class, uuid));
	}

	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	public Pdt find(String uuid) {
		return (Pdt) sessionFactory.getCurrentSession().get(Pdt.class, Integer.parseInt(uuid));
	...
	public Pdt find(Pdt model) {
		try {
			List<Pdt> list = getHibernateTemplate().findByExample(model);
			if (list.size() > 0) {
				return list.get(0);
			}
	...
	public List<Pdt> list() {
		return sessionFactory.getCurrentSession().createQuery("from Pdt").list();
	}
	...
	public Page findByPage(Page page, Map paramsMap) {
		...
		sb.append("from Pdt u where 1=1");
