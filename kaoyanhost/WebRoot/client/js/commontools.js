...
function showAttach(elid,filename){
    ...
    if(filename){
        if(isImg(filename)){
            ...
            imgel = "<a onclick='doloadFile();' >"+filename+"</a>";
        }
    }
...
function doloadFile(){
    myObj.openSite(fileurl+focusobj.attach);
}



function getChaDay(ndate){
    var str =ndate+' '+'00:00:00';
    str = str.replace(/-/g,"/");
    var date = new Date(str );
    var ritime = date.getTime();
    var cha = ritime-Date.now();
    var daycount = parseInt(cha/86400000);
    return daycount;
}