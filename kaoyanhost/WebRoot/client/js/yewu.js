...
//设置分类列表
    ...
    var p2 = {};
    p2.tpl ='<li class="gitem" onclick="toGood(%s);">'+
    '<img style="height: 130px;" src="'+fileurl+'%s">'+
    '<h2>%s</h2>'+
    //'<p>'+obj.note+'</p>'+
    '</li>';
    ...
    var p7tt = {};
    p7tt.tpl = '<li  onclick="toGood(%s);">'+
        '<img src="'+fileurl+'%s" style="height: 80px;">'+
        '<h2>%s</h2>'+
        '<p>%s</p>'+
        '</li>';
    p7tt.colums = ["id","img","gname","note"];
    $("#tuijianlist").data("property",JSON.stringify(p7tt));
...
function refreshGood(title,type){
    ...
    ajaxCallback("listGood",{stitle:title,stype:stype,order:paixu,sid:""},function(data){
        goodlist = data;
...
function toGood(id,flag){
    ...
    showAttach("showattach",obj.attach);
