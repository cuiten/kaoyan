...
var rootUrl = "http://localhost:8080/studytest18newscourse4vipchart2kaoyan/";
...
function  initswiper(){
    $("#sliderctn").empty();
    ajaxCallback("listGood",{btype:1},function(data){
        if(data.length>1){
            $("#sliderctn").append('<div onclick="toGood('+data[0].id+')" style="height: 150px; background-image: url('+fileurl+data[0].img+');background-size: 100% 100%;"  class="swiper-slide"></div>');
            $("#sliderctn").append('<div onclick="toGood('+data[0].id+')" style="height: 150px; background-image: url('+fileurl+data[1].img+');background-size: 100% 100%;"  class="swiper-slide"></div>');
        }
